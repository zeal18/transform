...
import io.github.zeal18.transform.internal.utils.MacroUtils
...
trait TransformerConfigSupport extends MacroUtils {
  ...
  def captureTransformerConfig(rawCfgTpe: Type): TransformerConfig = {
    ...
    if (cfgTpe =:= emptyT) {
      ...
      captureTransformerConfig(rest).fieldOverride(fieldNameTo, FieldOverride.RenamedFrom(fieldNameFrom))
