package io.github.zeal18.transform.internal.macros

import io.github.zeal18.transform.internal.*
import io.github.zeal18.transform.internal.utils.EitherUtils
...
trait TransformerMacros extends MappingMacros with TargetConstructorMacros with EitherUtils {
  ...
  def genTransformer[From: WeakTypeTag, To: WeakTypeTag](
    ...
    genTransformerTree(srcName, config)(From, To) match {
      ...
      case Right(transformerTree) =>
        config.derivationTarget match {
          case DerivationTarget.LiftedTransformer(f, _, _) =>
            q"""
               new _root_.io.github.zeal18.transform.TransformerF[$f, $From, $To] {
          ...
          case DerivationTarget.TotalTransformer =>
            q"""
               new _root_.io.github.zeal18.transform.Transformer[$From, $To] {
      ...
      case Left(derivationErrors) =>
        val errorMessage =
          s"""The Transform can't derive transformation from $From to $To
             |
             |${TransformerDerivationError.printErrors(derivationErrors)}
             |Consult $transformDocUrl for usage examples.
             |
             |""".stripMargin
              ...
              keyTransformer.target match {
                case DerivationTarget.LiftedTransformer(_, _, _) =>
                  q"""$wrapperErrorPathSupportInstance.addPath[$toKeyT](
                     ${keyTransformer.tree},
                     _root_.io.github.zeal18.transform.ErrorPathNode.MapKey($fnK)
                   )"""
                case DerivationTarget.TotalTransformer =>
              ...
              valueTransformer.target match {
                case DerivationTarget.LiftedTransformer(_, _, _) =>
                  q"""$wrapperErrorPathSupportInstance.addPath[$toValueT](
                      ...
                      _root_.io.github.zeal18.transform.ErrorPathNode.MapValue($fnK)
                   )"""
                case DerivationTarget.TotalTransformer =>
        ...
        q"""$wrapperSupportInstance.traverse[$To, ($FromInnerT, _root_.scala.Int), $ToInnerT](
            ...
            { case (${fn.name}: $FromInnerT, ${idx.name}: _root_.scala.Int) =>
              $wrapperErrorPathSupportInstance.addPath[$ToInnerT](
                $innerTransformerTree,
                _root_.io.github.zeal18.transform.ErrorPathNode.Index($idx)
              )
            }
          )
          """
        ...
        Right {
          TransformerBodyTree(
            q"""$errorPathSupport.addPath[${target.tpe}](
                 ${bodyTree.tree},
                 _root_.io.github.zeal18.transform.ErrorPathNode.Accessor(${accessor.symbol.name.toString})
               )""",
            config.derivationTarget,
          )
        }
      ...
      case DerivationTarget.LiftedTransformer(wrapperType, _, _) =>
        ...
        (implicitTransformerF, implicitTransformer) match {
          case (Some(localImplicitTreeF), Some(localImplicitTree)) =>
            c.abort(
              c.enclosingPosition,
              s"""Ambiguous implicits while resolving the Transform recursive transformation:
                 |
  ...
  def findLocalTransformerConfigurationFlags: Tree = {
    val searchTypeTree =
      tq"${typeOf[io.github.zeal18.transform.dsl.TransformerConfiguration[? <: io.github.zeal18.transform.internal.TransformerFlags]]}"
  ...
  private def findLocalImplicitTransformer(From: Type, To: Type, derivationTarget: DerivationTarget): Option[Tree] = {
    val searchTypeTree: Tree = derivationTarget match {
      case DerivationTarget.LiftedTransformer(f, _, _) =>
        tq"_root_.io.github.zeal18.transform.TransformerF[$f, $From, $To]"
      case DerivationTarget.TotalTransformer =>
        tq"_root_.io.github.zeal18.transform.Transformer[$From, $To]"
    }
  ...
  def findTransformerErrorPathSupport(wrapperType: Type): Option[Tree] =
    inferImplicitTpe(
      tq"_root_.io.github.zeal18.transform.TransformerFErrorPathSupport[$wrapperType]",
      macrosDisabled = true,
    )
  ...
  private def isDeriving(tree: Tree): Boolean =
    tree match {
      case TypeApply(Select(qualifier, name), _) =>
        qualifier.tpe =:= weakTypeOf[io.github.zeal18.transform.Transformer.type] && name.toString == "derive"
      case Apply(TypeApply(Select(qualifier, name), _), _) =>
        qualifier.tpe =:= weakTypeOf[io.github.zeal18.transform.TransformerF.type] && name.toString == "derive"
      case _ => false
    }
  ...
  private val transformDocUrl = "https://github.com/zeal18/transform/tree/main/docs"
}
