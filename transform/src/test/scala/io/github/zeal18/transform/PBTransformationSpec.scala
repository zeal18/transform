...
import utest.*
import io.github.zeal18.transform.examples.addressbook
import io.github.zeal18.transform.examples.order
import io.github.zeal18.transform.examples.pb
...
object PBTransformationSpec extends TestSuite {
  ...
  val tests = Tests {
    ...
    "not compile if target type is wrong for value class" - {
      ...
      compileError(""" addressbook.PersonName("John").transformInto[Int] """).check(
        "",
        "The Transform can't derive transformation from io.github.zeal18.transform.examples.addressbook.PersonName to Int",
      )

      compileError(""" addressbook.PersonId(5).transformInto[String] """).check(
        "",
        "The Transform can't derive transformation from io.github.zeal18.transform.examples.addressbook.PersonId to String",
      )

      compileError(""" addressbook.Email("john@example.com").transformInto[Float] """).check(
        "",
        "The Transform can't derive transformation from io.github.zeal18.transform.examples.addressbook.Email to Float",
      )
    }

    "transform enum represented as sealed trait hierarchy" - {

      (addressbook.MOBILE: addressbook.PhoneType).transformInto[pb.addressbook.PhoneType] ==>
        pb.addressbook.PhoneType.MOBILE

      (addressbook.HOME: addressbook.PhoneType).transformInto[pb.addressbook.PhoneType] ==>
        pb.addressbook.PhoneType.HOME

      (addressbook.WORK: addressbook.PhoneType).transformInto[pb.addressbook.PhoneType] ==>
    ...
    "transform bigger case classes" - {
      ...
      "PhoneNumber" - {
        ...
        addressbook.PhoneNumber("1234567", addressbook.HOME).transformInto[pb.addressbook.PhoneNumber] ==>
