...
import io.github.zeal18.transform.examples.colors1
import io.github.zeal18.transform.examples.colors2
import io.github.zeal18.transform.examples.foo
import utest.*
  ...
  val tests = Tests {
    ...
    "fix issue #121" - {
      ...
      compileError("Foo(None, FooNested(None)).into[Bar].transform").check(
        "",
        "derivation from foo.maybeString: scala.Option to scala.collection.immutable.Seq is not supported in the Transform!",
        "derivation from foo.nested.num: scala.Option to java.lang.String is not supported in the Transform!",
      )
    }
